...
export class EntityStoreService<T extends BaseDomain> {
   ...
   */
  public readonly navigateBackLabel$ = new BehaviorSubject<string>(null);
  /**
   * Заголовок страницы для вложенного редактора.
   */
  public readonly internalEditorTitle$ = new BehaviorSubject<string>(null);
  ...
  /**
   * Сбросить активную вкладку редактора.
   */
  public resetActiveTab(): void {
    this.innerActiveTab.next(null);
  }

  /**
   * Проверить, была ли открыта вкладка
   * @param id идентификатор вкладки
   */
  public isTabWasActivated(id: string): boolean {
    return this.innerWasActivatedTabs.includes(id);
  }
}
